    ...
    public function order_update(Request $request)
    {
        if (isset($request->do_method)) {
            if ($request->do_method == 'update') {
                ...
                $oldOrder = Order::where('order_id', $request->order_id)->first();
                $str = '';
                $columnArray = DB::select('SELECT column_name,column_comment FROM information_schema.columns WHERE
                table_schema = "idongp" AND table_name = "idp_order"');
                foreach($columnArray as $v0){
                    $column[$v0->column_name] = $v0->column_comment;
                }
                foreach($infoArr as $k => $v){
                    if($v != $oldOrder[$k]){
                        $str.= $column[$k].'已从'.$oldOrder[$k].'变为'.$v.',';
                    }
                }
                if($str != ''){
