    ...
    public function index(Request $request)
    {
        ...
        foreach($orders as $k => $order){
            if (!empty($order->launch_order_id)) {
                ...
                $orders[$k]['pro_url'] = route('product::item',['companyid'=>$companyid,'proid'=>$order->pro_id]);
                $orders[$k]['pro_img'] = $this->getImgArr('upload/proimg/'.$companyid.'_'.$order->pro_id.'/')[0];
                $orders[$k]['company_name'] = Company::where('company_id',Product::where('pro_id',$order->pro_id)
